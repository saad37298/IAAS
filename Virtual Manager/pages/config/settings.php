...
if (isset($_SESSION['key_path'])) {
  ...
  $sql = "SELECT name FROM  WHERE name = 'key_path';";
  $result = $conn->query($sql);

  if (mysqli_num_rows($result) == 0 ) {
    $sql = "INSERT INTO  (name, value) VALUES ('key_path', '$key_path');";
    $result = $conn->query($sql);
  } else {
    $sql = "UPDATE  SET value = '$key_path' WHERE name = 'key_path';";
    $result = $conn->query($sql);
  }
}
...
$sql = "SELECT value FROM  WHERE name = 'key_path' LIMIT 1;";
$result = $conn->query($sql);
if (mysqli_num_rows($result) != 0 ) {
  while ($row = $result->fetch_assoc()) {
    $key_path = $row['value'];
